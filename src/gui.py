...
class Gui:
    ...
    def fenToMatrix(self, fen: str):
        ...
        while zeile < 8:
            ...
            for i in range(len(tmpRow)):
                try:
                    value = int(tmpRow[i])
                    if (zeile == 0 or zeile == 7) and spalte == 0:
                        spalte += value + 1 if value != 0 else 1
                    else:
                        # mögliche Fälle im FEN String:
                        # x{r,b}0 | {r,b}0y | {r,b}{r,b} |
                        if value != 0 and spalte == 0:
                            spalte += value
                        elif value != 0 and spalte > 0 and spalte < 7:
                            spalte += value;
                    ...
                    match tmpVal:
                        case 0:
                            board[zeile][spalte] = figure
                        case 1:
                            board[zeile][spalte] += figure
                        case 4:
                            board[zeile][spalte] += figure if figure == 4 else -1
    ...
    def mapValueToColor(self, value: int):
        ...
        match value:
            case 1:
                return [self.__ROT]
            case 2:
                return [self.__ROT, self.__DARKRED]
            case 3:
                return [self.__ROT, self.__BLAU]
            case 4:
                return [self.__BLAU]
            case 5:
                return [self.__BLAU, self.__ROT]
            case 8:
                return [self.__BLAU, self.__DARKBLUE]
            case _:
                return []
    ...
    def zeichne_feld(self, x, y, farbe):
        ...
        x * self.BREITE / self.FELDER, y * self.HÖHE / self.FELDER, self.BREITE / self.FELDER, self.HÖHE / self.FELDER))
    ...
    def zeichneFigur(self, x, y, farbe, shift):
        pygame.draw.ellipse(self.screen, farbe, (
        x * self.BREITE / self.FELDER, y * self.HÖHE / self.FELDER + 25 + shift, self.BREITE / self.FELDER,
        self.HÖHE / (self.FELDER * 2)))
    ...
    def start(self):
        ...
        while running:
            ...
            for x in range(0, self.FELDER):
                self.zeichne_text(chr(x + 65), x * self.BREITE / self.FELDER + self.BREITE / self.FELDER / 2,
                                  self.HÖHE + 20, self.__SCHWARZ)
            ...
            for y in range(1, self.FELDER + 1):
                self.zeichne_text(str(9 - y), -10, (y - 1) * self.HÖHE / self.FELDER + self.HÖHE / self.FELDER / 2,
                                  self.__SCHWARZ)
