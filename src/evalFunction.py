...
class EvalFunction:
    ...
    def _scoreRating(self, startPos: np.uint64, targetmoves: list[np.uint64], board: list[np.uint64],
        ...
        if (startPos & board[GameState._ZARR_INDEX_R_PAWNS] & ~(
                board[GameState._ZARR_INDEX_B_KNIGHTS] | board[GameState._ZARR_INDEX_R_KNIGHTS]) != 0):
            targetScores.update(
        ...
        if (len(targetScores) == 0):
            raise ValueError("Error in MoveList, please check Zuggenerator, move=",
                             MoveLib.move(startPos, targetmoves[0], 3))
    ...
    def _materialPoints(self, board: list[np.uint64]):
        ...
        bp = bin(board[GameState._ZARR_INDEX_B_PAWNS] & (~board[GameState._ZARR_INDEX_R_KNIGHTS]))[2:].count("1") * \
             self._CONFIG_DICT[Config.MAT_PAWN]
        # count Blue Knights
    ...
    def computeOverallScore(self, moveList: list, board: list[np.uint64], printList=False,
                            returnSortedList=True) -> ScoredMoveList:
    ...
    def _computeActualPositionalPoints(self, board: list[np.uint64]):
        ...
        maxKnightsPos = board[
            GameState._ZARR_INDEX_R_KNIGHTS if maxPlayer == Player.Red else GameState._ZARR_INDEX_B_KNIGHTS]

        minPawnsPos = board[GameState._ZARR_INDEX_B_PAWNS if maxPlayer == Player.Red else GameState._ZARR_INDEX_R_PAWNS]
        minKnightsPos = board[
            GameState._ZARR_INDEX_B_KNIGHTS if maxPlayer == Player.Red else GameState._ZARR_INDEX_R_KNIGHTS]
        ...
        if (maxPawnPos):
            ...
            for figPos in FigPosStrings:
                positionPointsMax += self._CONFIG_DICT[
                    Config.TOTAL_SCORE_RATING_PAWN_RED if maxPlayer == Player.Red else Config.TOTAL_SCORE_RATING_PAWN_BLUE][
                    figPos]
        ...
        if (maxKnightsPos):
            ...
            for figPos in FigPosStrings:
                positionPointsMax += self._CONFIG_DICT[
                    Config.TOTAL_SCORE_RATING_KNIGHT_RED if maxPlayer == Player.Red else Config.TOTAL_SCORE_RATING_KNIGHT_BLUE][
                    figPos]
        ...
        if (minPawnsPos):
            ...
            for figPos in FigPosStrings:
                positionPointsMin += self._CONFIG_DICT[
                    Config.TOTAL_SCORE_RATING_PAWN_BLUE if maxPlayer == Player.Red else Config.TOTAL_SCORE_RATING_PAWN_RED][
                    figPos]
        if (minKnightsPos):
            ...
            for figPos in FigPosStrings:
                positionPointsMin += self._CONFIG_DICT[
                    Config.TOTAL_SCORE_RATING_KNIGHT_BLUE if maxPlayer == Player.Red else Config.TOTAL_SCORE_RATING_KNIGHT_RED][
                    figPos]
