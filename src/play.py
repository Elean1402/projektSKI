...
def call(state, output=False):
    ...
    depth = 5
    profile(lambda: search_instance.search(iterative_deepening=False, time_limit=5, depth=depth), 'alpha_beta',fen=state['board'], repetitions=1, depth=depth, move_output=True, include_move_count=True)
    

def board_test(state, output=False):
    # game_over = [DictMoveEntry.CONTINUE_GAME]
    bitboard = state['bitboards']
    move_generator = MoveGenerator(bitboard, useTakeback=True)
    ...
    if output:
        test = move_generator.genMoves(Player.Blue,bitboard,[DictMoveEntry.CONTINUE_GAME])
        for i in test:
            for j in i[1]:
